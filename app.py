...
if "p_auto_addr" not in st.session_state:
    st.session_state.p_auto_addr = ""

# 1. 開催日の選択（フォームの外に置くことで入力を即時反映させる）
st.subheader("【開催日】")
...
with col_d:
    selected_day = st.selectbox("日", list(range(1, 31)), index=datetime.now().day - 1)
...
if st.button("住所を検索する"):
    ...
    if not st.session_state.auto_addr:
        st.error("住所が見つかりませんでした。正しい番号を入力してください。")
...
with st.form("entry_form"):
    ...
    p_zip = st.text_input("親権者 郵便番号（7桁）")
    # 親権者側の住所は手動、または上記と同様のボタン方式にするのが確実ですが、ここでは入力欄のみ配置します
    parent_address = st.text_input("親権者 住所")
    parent_name = st.text_input("親権者 氏名")
    parent_phone = st.text_input("親権者 電話")

    st.divider()
    st.warning("【誓約事項】一字一句、資料通りに記載されます。")
    st.write("""
